/**
 * Test Uji Coba Endpoint Sinkronisasi Profil ASN (Worker Edge & Direct PHP Origin)
 * Format Output dan Validasi Kesamaan Response Berdasarkan .agents/api.md
 * File: tests/js/api-profil-sync.test.js
 */
...
describe('Uji Coba Endpoint Sinkronisasi Profil ASN: Worker Edge & Direct PHP Origin', () => {
    ...
    // =========================================================================
    // 1. TEST KONDISI DATA SALAH: TANPA HEADER AUTH / TOKEN DUMMY (NEGATIVE TEST)
    // =========================================================================
    test('1. Test kondisi token salah / tanpa header: output error 401 & kecocokan Worker vs PHP Origin', async () => {
        ...
        const workerTargetUrl = buildTargetUrl(WORKER_URL, `/api/profil/sync?cb=${Date.now()}`);
        ...
        const originTargetUrl = buildTargetUrl(ORIGIN_URL, `/api/profil/sync?cb=${Date.now()}`);
    ...
    // =========================================================================
    test('2. Test kondisi token valid: kecocokan response & payload access_token', async () => {
        expect(validAuthToken).toBeDefined();

        // --- 2.1 Worker Sync Profil ---
        const workerTargetUrl = buildTargetUrl(WORKER_URL, `/api/profil/sync?cb=${Date.now()}`);
        printLog(`\n[LANGKAH 2.1] POST (Token Sah Worker) -> ${workerTargetUrl}`);
        ...
        // --- 2.2 Direct PHP Origin Sync Profil ---
        const originTargetUrl = buildTargetUrl(ORIGIN_URL, `/api/profil/sync?cb=${Date.now()}`);
        printLog(`\n[LANGKAH 2.2] POST (Token Sah Origin) -> ${originTargetUrl}`);
        ...
        if (workerData.status === true && originData.status === true) {
            printLog('\n[VALIDASI KECOCOKAN STRUKTUR] Memeriksa kesamaan status, code, message, dan kunci data...');
            expect(workerData.status).toBe(originData.status);
            expect(workerData.code).toBe(originData.code);
            expect(workerData.message).toBe(originData.message);
            ...
            // Memastikan kunci objek response root dan objek data sama persis antara kedua server
            expect(Object.keys(workerData)).toEqual(Object.keys(originData));
            expect(Object.keys(workerData.data)).toEqual(Object.keys(originData.data));
            expect(Object.keys(workerData.data)).toEqual(['access_token']);
            ...
            // --- 2.4 Ekstrak & Bandingkan Payload JWT Token Hasil Sync ---
            printLog('\n[VALIDASI PAYLOAD JWT SYNC] Mengurai & membandingkan isi data klaim user di access_token...');
            ...
            printLog(`Decoded Worker JWT Payload: ${JSON.stringify(parsedWorker, null, 2)}`);
            printLog(`Decoded Origin JWT Payload: ${JSON.stringify(parsedOrigin, null, 2)}`);

            const workerUserData = parsedWorker.data || parsedWorker;
            const originUserData = parsedOrigin.data || parsedOrigin;

            // Memastikan seluruh data klaim user di dalam token sama persis antara Worker dan PHP Origin
            ...
            expect(workerUserData.role).toEqual(originUserData.role);
            expect(workerUserData.jenis_asn).toBe(originUserData.jenis_asn);
            expect(workerUserData).toEqual(originUserData);
        }
        ...
        expect(originData.message).toBe('Profil berhasil disinkronkan.');
        expect(originData.data).toBeDefined();
        expect(Object.keys(originData.data)).toEqual(['access_token']);

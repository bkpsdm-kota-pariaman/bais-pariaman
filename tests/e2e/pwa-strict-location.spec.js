...
test.describe('E2E Strict Location: Pengujian Aturan Lokasi Ketat (is_strict_location)', () => {

  test('Aturan Ketat Lokasi: Hadir Ditolak (Luar Radius) & Tidak Hadir Diizinkan', async ({ page, context }) => {
    // Set Geolocation jauh di luar radius Pariaman (misal: Jakarta -6.2088, 106.8456)
    await context.grantPermissions(['geolocation']);
    await context.setGeolocation({ latitude: -6.2088, longitude: 106.8456 });
    attachLogger(page, 'E2E Strict Location');
    test.skip(!ADMIN_USER || !ADMIN_PASS, 'Set ADMIN_USER dan ADMIN_PASS di environment variables.');
    test.setTimeout(180000);

    const timestamp = Date.now();
    const judulKegiatan = `UJI STRICT LOCATION ${timestamp}`;
    let kodeAkses = null;
    const todayStr = new Date().toISOString().split('T')[0];

    // =========================================================================
    // 1. ADMIN: LOGIN & BUAT JADWAL DENGAN STRICT LOCATION (RADIUS KETAT 50M)
    // =========================================================================
    ...
    await page.evaluate(() => {
      document.getElementById('geoLatLang').value = '-0.6264,100.1186';
    });
    await page.fill('#geoRadius', '50');

    // AKTIFKAN ATURAN KETAT LOKASI (Strict Location)
    logAction.check('Aktifkan Strict Location', '#addStrictLocation');
    await page.locator('#addStrictLocation').check();
    ...
    logAction.success(`Jadwal Strict Location berhasil dibuat. Kode: ${kodeAkses}`);
    ...
    // =========================================================================
    // 3. PENGUJIAN HADIR (DITOLAK KARENA STRICT LOCATION AKTIF & DI LUAR RADIUS)
    // =========================================================================
    ...
    // Klik Opsi Hadir untuk memicu pemeriksaan GPS lokasi ketat
    logAction.check('Pilih Opsi Hadir (Strict Location)', 'input[name="tipeKehadiran"][value="hadir"]');
    await page.click('input[name="tipeKehadiran"][value="hadir"]');
    await page.evaluate(() => {
      if (typeof pilihOpsiKehadiran === 'function') pilihOpsiKehadiran('hadir');
    });

    // PWA memeriksa lokasi, mendeteksi jarak > 50m dengan is_strict_location == 1, dan memicu SweetAlert error
    const swalStrictLocation = page.locator('.swal2-popup:has-text("Di Luar Lokasi"), .swal2-popup:has-text("Wajib Sesuai Lokasi")');
    await expect(swalStrictLocation).toBeVisible({ timeout: 12000 });
    logAction.success('SweetAlert "Di Luar Lokasi" berhasil muncul memblokir absensi Hadir!');

    // Tutup SweetAlert error
    await page.locator('.swal2-confirm').click();
    await expect(page.locator('#view-dashboard')).toBeVisible({ timeout: 15000 });
    logAction.success('PWA otomatis membatalkan form dan kembali ke dashboard setelah penolakan lokasi.');

    // =========================================================================
    // 4. PENGUJIAN TIDAK HADIR (DIIZINKAN WALAU DI LUAR RADIUS)
    // =========================================================================
    ...
    await page.fill('#keteranganIzin', `Uji Strict Location Tidak Hadir ${timestamp}`);
    ...
    await page.locator('#buktiIzin').setInputFiles({ name: 'bukti-location.png', mimeType: 'image/png', buffer: mockPngBuffer });
    ...
    logAction.success('PENGUJIAN STRICT LOCATION SELESAI 100% SUKSES!');
  });
});
